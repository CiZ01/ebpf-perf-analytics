...
void sample_usage(char *argv[], const struct option *long_options, const char *doc, int mask, bool error);
...
#endif
__attribute__((unused)) static inline char *safe_strncpy(char *dst, const char *src, size_t size)
{
    if (!size)
        return dst;
...
#define sample_init_pre_load(skel)                                                                                     \
    ({                                                                                                                 \
        ...
        sample_setup_maps((struct bpf_map *[]){                                                                        \
            skel->maps.rx_cnt, skel->maps.redir_err_cnt, skel->maps.cpumap_enqueue_cnt, skel->maps.cpumap_kthread_cnt, \
            skel->maps.exception_cnt, skel->maps.devmap_xmit_cnt, skel->maps.devmap_xmit_cnt_multi});                  \
    })

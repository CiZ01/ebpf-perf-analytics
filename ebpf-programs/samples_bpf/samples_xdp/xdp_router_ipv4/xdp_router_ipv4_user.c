...
#include <getopt.h>
#include <pthread.h>
#include "../xdp_sample/xdp_sample_user.h"
#include "xdp_router_ipv4.skel.h"

static const char *__doc__ = "XDP IPv4 router implementation\n"
...
static int mask = SAMPLE_RX_CNT | SAMPLE_REDIRECT_ERR_MAP_CNT | SAMPLE_DEVMAP_XMIT_CNT_MULTI | SAMPLE_EXCEPTION_CNT;
...
static void read_route(struct nlmsghdr *nh, int nll)
{
    ...
    for (; NLMSG_OK(nh, nll); nh = NLMSG_NEXT(nh, nll))
    {
        ...
        assert(bpf_map_update_elem(tx_port_map_fd, &route.iface, &route.iface, 0) == 0);
        if (rtm_family == AF_INET)
        {
            struct trie_value
            {
            ...
            if (route.dst_len == 32)
            {
                ...
                {
                    assert(bpf_map_delete_elem(exact_match_map_fd, &route.dst) == 0);
                }
                else
                {
                    if (bpf_map_lookup_elem(arp_table_map_fd, &route.dst, &direct_entry.arp.mac) == 0)
                        direct_entry.arp.dst = route.dst;
                    assert(bpf_map_update_elem(exact_match_map_fd, &route.dst, &direct_entry, 0) == 0);
                }
            }
            for (i = 0; i < 4; i++)
                prefix_key->data[i] = (route.dst >> i * 8) & 0xff;

            if (bpf_map_lookup_elem(lpm_map_fd, prefix_key, prefix_value) < 0)
            {
                for (i = 0; i < 4; i++)
                ...
                assert(bpf_map_update_elem(lpm_map_fd, prefix_key, prefix_value, 0) == 0);
            }
            else
            {
                if (nh->nlmsg_type == RTM_DELROUTE)
                {
                    assert(bpf_map_delete_elem(lpm_map_fd, prefix_key) == 0);
                ...
                else if (prefix_key->data[0] == prefix_value->prefix[0] &&
                         prefix_key->data[1] == prefix_value->prefix[1] &&
                         prefix_key->data[2] == prefix_value->prefix[2] &&
                         prefix_key->data[3] == prefix_value->prefix[3] && route.metric >= prefix_value->metric)
                {
                    continue;
                }
                else
...
static void read_arp(struct nlmsghdr *nh, int nll)
{
    ...
    for (; NLMSG_OK(nh, nll); nh = NLMSG_NEXT(nh, nll))
    {
        ...
        {
            if (bpf_map_lookup_elem(exact_match_map_fd, &arp_entry.dst, &direct_entry) == 0)
            {
                ...
                assert(bpf_map_update_elem(exact_match_map_fd, &arp_entry.dst, &direct_entry, 0) == 0);
                memset(&direct_entry, 0, sizeof(direct_entry));
            }
            if (nh->nlmsg_type == RTM_DELNEIGH)
            {
                assert(bpf_map_delete_elem(arp_table_map_fd, &arp_entry.dst) == 0);
            }
            else if (nh->nlmsg_type == RTM_NEWNEIGH)
            {
                assert(bpf_map_update_elem(arp_table_map_fd, &arp_entry.dst, &arp_entry.mac, 0) == 0);
            }
        }
...
static void usage(char *argv[], const struct option *long_options, const char *doc, int mask, bool error,
...
int main(int argc, char **argv)
{
    ...
    while ((opt = getopt_long(argc, argv, "si:SFvh", long_options, &longindex)) != -1)
    {
        switch (opt)
        {
        case 's':
    ...
    for (i = 0; i < total_ifindex; i++)
    {
        ...
        if (sample_install_xdp(skel->progs.xdp_router_ipv4_prog, index, generic, force) < 0)
            goto end_destroy;
    }
